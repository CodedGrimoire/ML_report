...
The deep models process RGB images directly and learn spatial features from the training data \cite{krizhevsky2012alexnet}. The updated experiments evaluate a CNN trained from scratch, ResNet50, ConvNeXt-Tiny, EfficientNet-B0, and an ensemble at $320\times320$ input resolution where applicable \cite{he2016resnet}, \cite{liu2022convnext}. Weighted sampling, class-weighted loss, threshold tuning on out-of-fold probabilities, and horizontal-flip test-time augmentation were used to address class imbalance and improve unsafe-class recall. Figure~\ref{fig:arch_cnn} shows the CNN architecture, and Table~\ref{tab:model_config} reports the combined configuration.
...
ResNet50 / ConvNeXt-Tiny & RGB $320\times320$ & WeightedRandomSampler, class-weighted loss, out-of-fold threshold tuning, horizontal-flip test-time augmentation. \\
...
This section reports the updated results. The most reliable comparison for deep models is the 5-fold stratified cross-validation over the 327 train+validation images, because the untouched 58-image holdout contains only about 15 unsafe samples and is therefore noisy. Table~\ref{tab:comparison} summarizes the headline cross-validated and baseline metrics, while Figs.~\ref{fig:paper_model_comparison} and~\ref{fig:paper_results_table} show the full held-out test comparison for all nine reported models. The ResNet50--ConvNeXt ensemble gives the strongest balanced cross-validated performance overall, while SVM remains the strongest classical HOG baseline by AUC.

\begin{table}[!t]
\centering
\caption{Updated performance comparison. Deep-model rows use cross-validation; classical rows use the updated holdout test.}
\label{tab:comparison}
\footnotesize
\setlength{\tabcolsep}{2pt}
\begin{tabular}{lccccc}
\toprule
\textbf{Model} & \textbf{Eval.} & \textbf{Acc.} & \textbf{Unsafe R} & \textbf{Unsafe P} & \textbf{AUC} \\
\midrule
Ensemble (ResNet50+ConvNeXt) & CV & \textbf{0.917} & 0.807 & 0.859 & \textbf{0.929} \\
ResNet50 & CV & 0.905 & \textbf{0.843} & -- & 0.919 \\
ConvNeXt-Tiny & CV & 0.905 & 0.819 & -- & 0.923 \\
EfficientNet-B0 & CV & 0.865 & 0.783 & -- & 0.884 \\
SVM (RBF) & Test & 0.793 & 0.667 & 0.588 & 0.836 \\
Logistic Regression & Test & 0.707 & 0.733 & -- & 0.822 \\
Gaussian Naive Bayes & Test & 0.672 & 0.600 & -- & 0.698 \\
CNN & Test & 0.914 & 0.667 & 1.000 & 0.822 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{figures/fig3_results_table.png}
    \caption{Rendered full held-out test results table for all nine reported models.}
    \label{fig:paper_results_table}
\end{figure*}
...
The updated result set compares Logistic Regression, Gaussian Naive Bayes, SVM with an RBF kernel, a CNN trained from scratch, ResNet50, ConvNeXt-Tiny, EfficientNet-B0, and a ResNet50--ConvNeXt ensemble. The ensemble achieved the strongest balanced cross-validated result with 91.7\% accuracy, 80.7\% unsafe recall, and 0.929 AUC. For safety-oriented screening, the high-recall operating mode increased unsafe recall to 95.2\% at 74.6\% accuracy, while zero-miss mode reached 100\% unsafe recall with many false alarms.
